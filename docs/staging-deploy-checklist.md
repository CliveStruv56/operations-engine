...
1. Postgres up → create roles → `alembic upgrade head` (as owner) →
   `python -m app.groundwork.seeds` (reference data, idempotent).
...
4. Worker up — **use the rebuilt image** (job_timeout 3600, WeasyPrint +
   pango libs, health-card function). `docker compose build worker` needs a
   healthy Docker; see the rebuild note in the session log.

...
| 3 | ~~Fix `test_every_endpoint_requires_token` so it walks the included routers~~ **Done, 3 Sep.** The walk recurses through `_IncludedRouter.original_router` and now checks all 186 protected routes; `test_route_walk_sees_the_whole_app` fails if it ever finds fewer than 150, so it cannot silently empty itself again. Verified by negative control — a router included without an auth dependency is caught | The test that guarantees "no endpoint is reachable without a JWT" currently checks **zero endpoints** and passes. There is no hole today — I walked all 190 routes and only the 2 health and 2 HMAC digest routes are open, all by design — but the net that would catch tomorrow's mistake is dead, and dead silently | `tests/test_auth.py` |
| 4 | ~~`disallowedElements={["img"]}` in `markdown.tsx`~~ **Done, 3 Sep — both halves.** The worker's four renderers go through `worker/pdf.py`, which refuses every non-`data:` URL, and `answer_pdf` disables the markdown image rule; `AnswerMarkdown` now drops the `img` element outright. **The prompt-side half of LLM-1 is still open** — retrieved material is still interpolated into the *system* message with unescaped `<vault-excerpts>` tags | Two ends of the same hole: retrieved third-party content sits in the *system* prompt, and both the browser and the PDF renderer would fetch a remote image the model was talked into emitting | `apps/web/components/markdown.tsx`, `components/markdown.test.tsx` |
| 5 | ~~Move the JWKS lookup off the event loop~~ **Done, 3 Sep.** The lookup runs in `anyio.to_thread.run_sync` with a 5s `PyJWKClient` timeout, and a bounded 60s negative cache of missed `kid`s means an unknown key costs one fetch, not one per request. `tests/test_auth_jwks.py` covers the branch the rest of the suite never reaches | A synchronous, 30s-timeout HTTPS fetch runs on the event loop *before any signature is checked*, on every authenticated route. Tokens carrying random `kid` values miss the cache and force one blocking round trip each — no credential needed. The cheapest way to take the whole API down | `app/auth.py`, `tests/test_auth_jwks.py` |
| 6 | ~~`set -o pipefail` + a size floor in `backup.sh`~~ **Done, 3 Sep.** `pipefail`, a 20 KB floor (`BACKUP_MIN_BYTES`) checked before any upload, and an `EXIT` trap that logs the failure and emails it when `RESEND_API_KEY` + `BACKUP_ALERT_EMAIL` are set. Verified in the real image against a live Postgres: a bad password now exits 1 with nothing uploaded, an empty database is refused at 391 bytes, and a good run still uploads both. The old script, same case, uploaded a **20-byte** archive over `daily-Thu.sql.gz` and printed "backup complete" | Today a failed `pg_dump` writes an empty-but-valid gzip, passes `gzip -t`, overwrites the day's slot and exits 0. Seven bad nights would silently replace every rolling backup | `infra/backup/backup.sh` |
